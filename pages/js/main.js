/**
 * Controls main site functions like asset loading and initialization
 */
...
// Static paths for assets - relative to the current page
...
export async function fetchJSON(url, opts = {}) {
	...
	try {
		...
		// Only log for page issues, not network
...
export async function getSiteConfig() {
	...
	const data = await fetchJSON(`${DATA_PATH}site.json`)
...
export async function getPages() {
	...
	const data = await fetchJSON(`${DATA_PATH}pages.json`)
	return data ?? fallback
}
...
export async function getFtpClients() {
	return fetchJSON(`${DATA_PATH}neuro-ftp-clients.json`)
}
